...
Он создаёт `ami-qwen3:1.7b-nothink` на существующих весах Qwen3: один лёгкий рантайм с контекстом 8K используется для классификации и простых ответов. Это предотвращает пустые ответы через OpenAI-совместимый API и одновременную загрузку тяжёлых моделей на MacBook 16 ГБ. Сложные задачи передаются подписочным CLI. Скрипт настраивает Hermes и OpenCode, синхронизирует skills и подключает MacBook worker к Mac Mini через закрытый SSH-туннель. Broker token остаётся вне репозитория.

В OpenCode: `ami` маршрутизирует задачи, `local-chat` отвечает только локально, `local-deep` использует установленную Gemma и работает медленнее. После обновления конфигурации перезапустите OpenCode и начните новый чат: старый чат может сохранять прежний выбор модели. Неисправный Ponytail и дублирующий загрузчик skills исключены из MacBook-профиля; стандартные skills и MCP сохранены.
...
|--------|------------|-------------|
| `qwen3:1.7b` | быстрая классификация запросов | Не выполняет изменения репозиториев |
| `ami-qwen3:1.7b-nothink` | быстрый чат и маршрутизация на MacBook 16 ГБ | Контекст 8K; только простые ответы, сложные задачи делегируются |
