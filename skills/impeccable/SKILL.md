...
## Amori Deployment Policy

- The user's request, repository conventions, and the active agent's safety rules override this skill.
- Use subscription-authenticated Codex or Claude CLI only. Never request, configure, or fall back to
  metered OpenAI or Anthropic API keys.
- Do not enable hooks, install the npm CLI or browser extension, edit CSP, or persist Impeccable
  project metadata unless the user explicitly asks for that specific change.
- Keep `ui-design-research` supplementary. Do not load Taste Skill alongside this skill unless the
  user explicitly requests an experimental Taste pass.
- For operational products and dashboards, choose `Operate`; task clarity and scanability outrank
  decorative ambition.
